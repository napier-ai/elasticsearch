...
// Rewrite
. A coordinating node receives the cross-cluster search API request.

https://drive.google.com/file/d/1n1U3fqEp3zuJLBpmunYRoSv2hZ2WEAA9/view
[[Image 9]]

. The coordinating node fetches shard and index information from each remote
cluster.

[[Image 10]]

. Each cluster performs the search request independently, reducing and fetching
the results. Each cluster then sends its results to the coordinating node.

[[Image 11]]

. After receiving results from each cluster, the coordinating node performs a
final reduction and returns the final results in the search API response.
...
. The coordinating node sends a <<search-shards,search shards>> API request to each remote cluster.
This request returns information about information about their corresponding
remote indices involved in the search request and the shards where their data
is located to the node.

[[Image 15]]

. The coordinating node sends one request to each shard involved, each shard executes the query 

[[Image 16]]

. each shard executes the query and returns its own results.

[[Image 17]]

. The coordinating node fetches the results from each shard, which are reduced.

[[Image 17]]

 which are then reduced (and fetched, depending on the
<<request-body-search-search-type, search type>>) by the CCS coordinating node.

. The coordinating node performs a
final reduction and returns the final results in the search API response.
